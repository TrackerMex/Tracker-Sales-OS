...
## 2026-07-16 — Feature 73: migración de iconografía HugeIcons → reicon-react

- El usuario instaló `reicon-react` y pidió migrar todos los iconos de la app. Se creó la feature 73 en `feature_list.json` con su CHECKPOINT.
- **Prerequisito no pedido pero bloqueante**: al dimensionar la feature se detectó que CI corría `npm ci` contra un `package-lock.json` desactualizado, mientras los Dockerfiles de dev y prod ya usaban `pnpm install` con `pnpm-lock.yaml`. CI validaba un árbol de dependencias distinto al que se despliega. Se unificó todo a pnpm: workflow con `pnpm/action-setup@v4` + `--frozen-lockfile`, `packageManager: pnpm@10.33.4` en ambos `package.json`, y se eliminaron los 3 lockfiles muertos (`frontend/bun.lock`, `frontend/package-lock.json`, `backend/package-lock.json`). De paso se corrigió que `pnpm test -- --runInBand` hacía que Jest leyera el flag como pattern y reportara 0 tests: un falso rojo en CI. Documentado en `docs/verification.md`.
- **Exploración (2 subagentes)**: corrigió el alcance que el Líder había estimado a ojo. Eran 52 instancias / 23 iconos, no 48/24 — `showIcon` era un falso positivo de grep y los iconos del sidebar (`nav-items.tsx`) son SVG inline, no HugeIcons. La exploración de la API encontró que reicon **sí** reenvía `className`/`ref`/`data-*` (no era el bloqueante temido) pero tiene 3 problemas reales verificados en `createIcon.js`: `style="color:currentColor"` inline que anula las clases `text-*`, `strokeWidth` no-op en 1565/2674 iconos, y 154 iconos con `scale(1.33333)` que multiplican el grosor ×1.33.
- **Decisión del usuario**: el Líder generó una comparación visual lado a lado de los 23 iconos (ambas librerías renderizadas con `react-dom/server`) antes de comprometer 24 archivos, porque la migración no era mecánica y tenía regresiones inevitables. Con eso a la vista, el usuario eligió **migrar con wrapper**.
- **Implementación**: `frontend/src/shared/components/Icon.tsx` como único punto de acoplamiento, 23 exports semánticos, `strokeWidth` por icono (nunca global), `forwardRef`. 3 desviaciones declaradas, las 3 juzgadas correctas por el Reviewer; la más importante: no aplicar `style={{color: undefined}}` incondicional (habría matado los 7 colores hex de los módulos) sino `style={{ color, ...style }}`.
- **Review**: PASSED 18/18 verificables, sin bloqueantes. Corrigió dos imprecisiones del reporte del Implementer (6 instancias de `color="currentColor"`, no 7; el rojo del item destructivo nunca estuvo en riesgo) y detectó 13 call-sites que pasaban `strokeWidth={1.8}` y ahora rinden 2.0, sin documentar.
- **Smoke del Líder**: requirió rebuild del contenedor `frontend` (monta sólo `./frontend/src`, así que `node_modules` no tenía reicon). Verificado en navegador con sesión admin: 0 errores de consola en 12 rutas, y con `getComputedStyle` los dos fallos silenciosos descartados — **0 svgs con `style` inline de color** y **0 iconos con tamaño 0**.
- **Riesgos aceptados**: grosores mezclados 2.0 (stroke) / ~1.5 (fill), sin arreglo posible por ser geometría del path; tick del checkbox de 3 → ~1.5. Los 4 mapeos dudosos se cambian con una línea en `Icon.tsx`.
- **Hallazgo lateral preexistente** (no de esta feature): `dropdown-menu.tsx:46` anula con `!important` el `text-destructive` de los items, por eso "Eliminar cliente" no se ve rojo.
