...
3. Dashboard: Prettier exit 0; budgets `DashboardPage 0/2`, `SellerSemaphoreTable 1/2`, `LeaderboardTable 0/2`, `KPICard 0/1`, `AlertsPanel 0/1`; module typecheck exit 0. Progress width is represented with `scaleX` and `origin-left transition-transform`.
4. Activities: Prettier exit 0; budgets `ActivitiesPage 2/2`, `ActivityHistoryModal 0/1`, `ActivityForm 2/2`; module typecheck exit 0. Progress width is represented with `scaleX` and `origin-left transition-transform`.
...
The final `rg -c 'style=\{\{'` equivalent count is 6 of the allowed 33. Every individual file is within its budget.
