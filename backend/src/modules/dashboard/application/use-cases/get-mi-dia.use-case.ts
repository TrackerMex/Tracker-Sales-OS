  ...
  async execute(sellerId: string): Promise<MiDiaDto> {
    ...
    const [
      ...
      this.activityRepo
        ...
        .andWhere(
          'a.executed_at >= :todayStart AND a.executed_at < :todayEnd',
          {
            todayStart,
            todayEnd,
          },
    ...
    if (callsToday > 5 && tomorrowTasksCount === 0) {
      coachTips.push(
        'Tienes buen ritmo de llamadas. Agenda al menos 5 tareas para mañana.',
      );
    }
    if (pointsToday < 10 && callsToday < 3) {
      coachTips.push(
        'Día con pocos puntos. Prioriza llamadas y reuniones de mayor valor.',
      );
    }
    if (overdueCount > 0) {
      coachTips.push(
        `Tienes ${overdueCount} seguimiento(s) vencido(s). Atiéndelos primero.`,
      );
    }
    if (newProspectsToday === 0) {
      coachTips.push(
        'Sin prospectos nuevos hoy. Considera prospectar antes de que termine el día.',
      );
    }
