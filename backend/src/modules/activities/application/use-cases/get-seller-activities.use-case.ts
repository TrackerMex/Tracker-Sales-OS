...
interface GetSellerInput extends GetActivitiesQueryDto {
  sellerId: string;
}

@Injectable()
export class GetSellerActivitiesUseCase implements IUseCase<
  GetSellerInput,
  { data: ActivityDto[]; total: number }
> {
  constructor(
  ...
  async execute(
    ...
    return {
      data: data.map((activity) => ActivityDto.fromEntity(activity)),
      total,
    };
  }
}
